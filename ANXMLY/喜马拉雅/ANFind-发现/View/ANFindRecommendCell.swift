...
class ANFindRecommendCell: UITableViewCell {
    ...
    var model:StreamModel?{
        didSet{
            ...
            iconView.kf.setImage(with: URL.init(string: item.avatar! as String))
            namelb.text = item.nickname
            let text = item.content ?? ""
            self.contentLb.text =  text as String
            self.contentLb.snp.updateConstraints { (make) in
                make.height.equalTo(contentTextHeight(text: text as String))
            }
            collectionView.reloadData()
            timeLb.text = String().updateTimeToCurrennTime(timeStamp:Double(item.issuedTs ?? 0))
            goodLb.text = String().stringFromPowerInt(count: item.likesCount ?? 0)
             commentLb.text = String().stringFromPowerInt(count: item.commentsCount ?? 0)
    ...
    func cellHeight(model:StreamModel) -> CGFloat {
        var result:CGFloat = 0.0
        
        let text = model.content
        
        result += contentTextHeight(text:text! as String)
    ...
    func collectionHeight(model:StreamModel) -> CGFloat {
...
extension ANFindRecommendCell:UICollectionViewDataSource,UICollectionViewDelegate{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let item = model?.picUrls![indexPath.row]
        cell.iconView.kf.setImage(with: URL.init(string:item!.thumbnailUrl! as String ))
        return cell
    }
